...
execute as @e[tag=42.tag.portal.turret.target_current,limit=1,sort=nearest] at @s anchored eyes rotated 0 0 positioned ^ ^-.25 ^ run tp @e[tag=42.tag.portal.turret.laser_find] ^ ^ ^
...
execute unless entity @e[tag=42.tag.portal.turret.current] at @s run function 42:portal/cube/turret/target/range2