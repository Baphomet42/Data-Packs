...
execute as @e[scores={42.obj.portal.lvl=1..},type=!player] at @s unless entity @s[tag=42.tag.portal.wire.lbl] if score @s 42.obj.portal.lvl = #42.var.portal.dummy 42.obj.portal.lvl run summon area_effect_cloud ~ ~ ~ {Tags:["42.tag.summon"],Duration:30,DurationOnUse:0,custom_particle:{type:"minecraft:flash",color:[1,1,1,1]},Radius:.25d,RadiusOnUse:0,RadiusPerTick:0,WaitTime:0}
